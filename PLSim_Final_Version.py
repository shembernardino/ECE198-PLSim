...
def Input_Fields(window):   
    ...
    Label9 = Label(window, text="Map Setting", bg="black", fg="white", font="none 12 bold")
    Label9.place(x=In_Pam_x, y=In_Pam_y+110)
    ...
    #Scaling Factor Entry Box
    textentry3 = Entry(window, width=20, bg="white")
    textentry3.place(x=Dom_Pam_x+190, y=Dom_Pam_y+30)

    #Maximum Link Budget
    Label6 = Label(window, text="Max Value:", bg="black", fg="white", font="none 10 bold") 
    Label6.place(x=Dom_Pam_x+20, y=Dom_Pam_y+60)

    #Maximum Link Budget Entry Box
    textentry4 = Entry(window, width=20, bg="white")
    textentry4.place(x=Dom_Pam_x+190, y=Dom_Pam_y+60)

    #Foliage Density
    Label10 = Label(window, text="Preferred Path loss Model for Foliage:", bg="black", fg="white", font="none 12 bold") 
    Label10.place(x=Dom_Pam_x, y=Dom_Pam_y+110)
    ...
    radio3 = Radiobutton(window, text="NLOS", variable=cx, value=1) 
    radio3.place(x=In_Pam_x+20, y=In_Pam_y+140)
    radio4 = Radiobutton(window, text="LOS", variable=cx, value=2) 
    radio4.place(x=In_Pam_x+100, y=In_Pam_y+140)
    #radio5 = Radiobutton(window, text="Urban - Uneven", variable=cx, value=3) 
    #radio5.place(x=In_Pam_x+210, y=In_Pam_y+140)

    gx = IntVar()

    #Foliage Density
    radio6 = Radiobutton(window, text="MED", variable=gx, value=1) 
    radio6.place(x=Dom_Pam_x+70, y=Dom_Pam_y+140)
    radio7 = Radiobutton(window, text="Tewari", variable=gx, value=2) 
    radio7.place(x=Dom_Pam_x+150, y=Dom_Pam_y+140)
    ...
    return textentry1, textentry2, textentry3, cx, textentry4, gx
...
def Determine_Path_Loss(image, list, list_, sheet, list_2, list_3):
    ...
    def COST_231_Walfisch_Ikegami_Model(tuple1, int1, int2, tuple2, int3, int4):        #Model does not include free space path loss yet; Add fspl in main code
        ...
        if type_of_environment == 1:             
            k_f = -4 + 0.7*(((f/1000000)/925)-1)
        elif type_of_environment == 2 or type_of_environment == 3: 
    ...
    while counter < len(list):
        ...
        if np.all(color_values[counter] == white):  
            ...
            if counter == 0:
                ...
                total_cell_value = Foliage_Path_Loss + Free_space_path_loss
                #current_cell.value = Foliage_Path_Loss + Free_space_path_loss

                if type_of_environment == 1 and foliage_density == 1:
                    correction_factor = -53.1426*(math.log10(real_distance)) + 172.9272
                    current_cell.value = total_cell_value + correction_factor

                elif type_of_environment == 2 and foliage_density == 1:
                    correction_factor = 9.5332*(math.log10(real_distance))
                    current_cell.value = total_cell_value + correction_factor

                elif type_of_environment == 1 and foliage_density == 2:
                    correction_factor = -2.9139*(math.log10(real_distance))
                    current_cell.value = total_cell_value + correction_factor

                elif type_of_environment == 2 and foliage_density == 2:
                    correction_factor = 6.7408*(math.log10(real_distance))
                    current_cell.value = total_cell_value + correction_factor
                

                if current_cell.value > max_loss_value:
                    current_cell.value = max_loss_value
                ...
                Free_space_path_loss = 32.44 + 20*(math.log10(real_distance/1000))  + 20*(math.log10(f/1000000))
                total_cell_value = COST_231_model_total + Free_space_path_loss
                ...
                Free_space_path_loss = 32.44 + 20*(math.log10(real_distance/1000))  + 20*(math.log10(f/1000000))
                total_cell_value = COST_231_model_total + Foliage_Path_Loss + Free_space_path_loss
            ...
            elif gray_flag == False and green_flag == False:
...
def Canvas_Operations(window, canvas, list1):
    ...
    def save_image():
        ...
        a = list1[3].get()
        if a == "":
            print("Map profile environment is set to default")
        elif a != "":
            global type_of_environment
            type_of_environment = int(list1[3].get())

        a = list1[4].get()
        if a == "":
            print("No max value input")
        elif a != "":
            global max_loss_value
            max_loss_value = int(list1[4].get())

        a = list1[5].get()
        if a == "":
            print("No Foliage Density input")
        elif a != "":
            global foliage_density
            foliage_density = int(list1[5].get())
        ...
        if type_of_environment == 1: 
            print("Map Setting is: NLOS")
        elif type_of_environment == 2:
            print("Map Setting is: LOS")
        #elif type_of_environment == 3:
        #    print("Map Setting is: Urban - Uneven")

        if foliage_density == 1:
            print("Preferred Model is: Modified Exponential Decay Model")
        elif foliage_density == 2:
            print ("Preferred Moedl is: Tewari Model")

        print("Max Value is:", max_loss_value)
...
def load_building_coords(window, canvas):
    ...
    x = []
    y = []
    filename = filedialog.askopenfilename(initialdir="G:/My Drive/ECE 198/Simulator Program Files", title="Select a File", filetypes=(("txt files", "*.txt"),("all files", "*.*")))
